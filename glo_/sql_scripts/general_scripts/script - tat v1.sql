...
,round((extract(epoch from bsl.activation_date)::numeric - extract(epoch from bsl.create_date)::numeric)/60 ) as mttc2
,round((extract(epoch from bsl.activation_date)::numeric - extract(epoch from bsl.create_date)::numeric)/60,2 ) as mttc3
...
and (bsl.reg_type like 'NM%' or bsl.reg_type like 'AR%' or bsl.reg_type in ('MPI','VNI'))
