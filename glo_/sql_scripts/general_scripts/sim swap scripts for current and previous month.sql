...
select u.user_id, s.checker_user_id, s.device_tag as kit_tag, count(s.pk) as count_of_swapped, Extract(day from s.completion_date)  as day_in_month 
...
and extract(year from s.completion_date) = extract(year from now )
and extract(month from s.completion_date) = extract(month from now )
and date(s.completion_date) <= date(now)-1
group by u.user_id, s.checker_user_id, s.device_tag, Extract(day from s.completion_date), Extract(month from s.completion_date)
), mg as (
...
previous_month as (with days as (
select 	1   as daily_	from dual union all
select 	2	from dual union all
select 	3	from dual union all
select 	4	from dual union all
...
join km_user u on lower(u.email_address) = lower(s.agent_email)
where (s.swap_status) = 'SWAPPED' and (s.check_status) = 'CHECKED'
and date(s.completion_date) between date(date_trunc('month', current_date - interval '1' month)) and
date(date_trunc('month', current_date)-1)
group by u.user_id, s.checker_user_id, s.device_tag, Extract(day from s.completion_date), Extract(month from s.completion_date)
), mg as (
select er.user_id, er.checker_user_id, er.kit_tag, er.count_of_swapped, d.daily_ as days_in_month
from days d 
