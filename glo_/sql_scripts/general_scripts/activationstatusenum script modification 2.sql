with t1 as (
select bsl.msisdn,ku.email_address ,ku.dealer_fk,na.fsa_user_fk,na.kyc_dealer_fk, ku.create_date ,er."name" netbook_id, 
bsl.sim_serial sim_serial, 
		dd.DDA32 as STATUS,bsl.create_date as REGISTRATION_DATE,
	case when bsl.activationstatusenum is null then 'PENDING ACTIVATION' else bsl.activationstatusenum end as ACTSTATUS,
	/*case when bsl.activation_date is null then bsl.create_date else bsl.activation_date end as*/ bsl.activation_date,
	kd."name" dealer,
	md.state_of_registration as/*s."name"*/ state , ku.first_name||' '||ku.surname agent ,--na.node_fk ,
	round((extract(epoch from case when bsl.activation_date is null then bsl.create_date else bsl.activation_date end )::numeric - extract(epoch from bd.bd_part_key)::numeric)/60,2) ACT_DATE_DIFF
,row_number()over(partition by bsl.msisdn order by bsl.create_date desc )
from bfp_sync_log bsl
join user_id u on u.unique_id = bsl.unique_id 
join basic_data bd on bd.user_id_fk = u.id 
join dynamic_data dd on dd.basic_data_fk = bd.id 
join meta_data md on md.basic_data_fk = bd.id 
join enrollment_ref er on bsl.enrollment_ref_fk = er.id 
join node n on n.enrollment_ref = er.id 
join node_assignment na on na.node_fk = n.id
--join sms_activation_request sar on sar.unique_id = bsl.unique_id 
--join state s on s.id = sar.state_id 
left join km_user ku on lower(ku.email_address) = lower(bsl.agent_email_address)
--left  
 join kyc_dealer kd on kd.pk = na.kyc_dealer_fk 
where /*sar.phone_number = bsl.msisdn 
and */(bsl.msisdn_compliance_status not in ('BARRED','BAR_48','NCC_BAR') or BSL.msisdn_compliance_status is null)
AND bsl.bfpsyncstatusenum = 'SUCCESS'
and (bsl.reg_type like 'NM%' or bsl.reg_type like 'AR%' or reg_type in ('MPI','VNI','CN','CAR'))
and (bsl.activationstatusenum is null or BSL.activationstatusenum = 'ACTIVATED')
and date(bsl.create_date) >= '2021-11-1'
and date(bsl.create_date) <= '2021-11-13'
and kd."name" like  'AREWA MULTI-LINKS ENTERPRISES'
--and er.name like  ?
--and md.state_of_registration like ?
) 
select * from t1 where row_number =1 --order by actstatus desc 