...
from new_kd_6 where state not in (select name from  state);
...
select email_address, count(1) from kyc_dealer where date(create_date)=date(now()) group by 1 order by 2 desc;

update kyc_dealer 
set email_address = nextval('hibernate_sequence')||email_address
where email_address in ('udgodswilledem@yahoo.com');

UPDATE new_kd_6 n 
SET email = link.email_address
from kyc_dealer link
where link.deal_code = n.code
and n.email in ('udgodswilledem@yahoo.com')
;
...
and not exists (
select email_address from km_user ku where k.email_address = ku.email_address
)
;
...
update kyc_dealer a 
set
	a.address = (select max(b.address) from new_kd_6 b where b.code = a.deal_code),
	a.contact_address = (select max(b.address) from new_kd_6 b where b.code = a.deal_code), 
	a.mobile_number = (select max(b.phone_number) from new_kd_6 b where b.code = a.deal_code)
where a.email_address in (select email from new_kd_6)
and date(a.create_date) = date(now());
