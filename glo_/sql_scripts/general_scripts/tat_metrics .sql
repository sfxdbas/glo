...
,abs(round((extract(epoch from bsl.activation_date)::numeric - extract(epoch from bd.bd_part_key)::numeric)/60 )) as mtta2
,abs(round((extract(epoch from bsl.activation_date)::numeric - extract(epoch from bd.bd_part_key)::numeric)/60,2 )) as mtta3
,bsl.activation_date - bsl.create_date as mttc
,abs(round((extract(epoch from bsl.activation_date)::numeric - extract(epoch from bd.bd_part_key)::numeric)/60 )) as mttc2
,abs(round((extract(epoch from bsl.activation_date)::numeric - extract(epoch from bd.bd_part_key)::numeric)/60,2 )) as mttc3
