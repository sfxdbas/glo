...
--back region and zone, territory, state, lga  up
--change entries in region 
--make region and zone remain the same
-- work on territory 
--update state 
--update lga
select distinct sales_territory from dummy_01042021 order by  1 ;
...
select * from update_state_01042021 s ;
select * from state s ;
...
select * from territory t where name   in (select sales_territory from dummy_01042021 d2)
order by id 
;

--select * from state where id = 11;
