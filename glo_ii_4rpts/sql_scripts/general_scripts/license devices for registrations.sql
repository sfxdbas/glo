select * from new_kd_9;
--update new_kd_9 
--set state = 'RIVERS'
--where state in ('Rivers-1', 'Rivers 1');

select * from state;
--
--alter table new_kd_9 rename channel_partner_name to partner_name;
--alter table new_kd_9 rename trade_code to code;
--alter table new_kd_9 rename geopolitical_state to state;
--alter table new_kd_9 rename phone_numer to phone_number;
--alter table new_kd_9 rename partner_email to email;

select distinct(upper(address)), upper(address),code,max(email) email,phone_number,upper(partner_name)
from new_kd_9
group by 1,2,3,5,6
order by 3
;
...
select 
	distinct(upper(address)), upper(address),n.code,max(email) email,phone_number,upper(partner_name),
		s.zone_fk, 195456097, 6,
		hibernate_sequence.nextval, true, now(), false,now()
from new_kd_9 n  join  state s 
...
from new_kd_9 ts left join state on
